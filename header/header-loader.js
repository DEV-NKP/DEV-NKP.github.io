...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const isFirstLoad = !sessionStorage.getItem('loadingOverlayShown');
    
    // Load header HTML
    fetch('./header/header.html')
        .then(response => response.text())
        .then(html => {
            // If not first load, remove the loading overlay from the HTML
            if (!isFirstLoad) {
                ...
                const loadingOverlay = tempDiv.querySelector('.header-loading-overlay');
                if (loadingOverlay) {
...
function initializeHeader(isFirstLoad) {
    ...
    // Add flash animation to contact button
    if (contactButton) {
    ...
    // Random loading messages for inside laptop
    const laptopMessages = [
        "Setting things in motion...",
        "Almost there...",
        "Bringing everything together...",
        "Compiling awesome portfolio...",
        "Preparing developer experience..."
    ];
    
    // Random initializing words
    const initializingWords = [
        "INITIALIZING...",
        "LOADING...",
        "PREPARING...",
        "COMPILING...",
    ...
    // Set random loading messages
    function setRandomLoadingMessages() {
        const loadingText = document.getElementById('header-loading-text');
        const loadingMessage = document.getElementById('header-loading-message');
        
        if (loadingText && loadingMessage) {
            const randomMessage = laptopMessages[Math.floor(Math.random() * laptopMessages.length)];
            const randomWord = initializingWords[Math.floor(Math.random() * initializingWords.length)];
    ...
    // ======== MODIFIED LOADING OVERLAY LOGIC ========
    // Only handle loading overlay if it's the first load
    if (isFirstLoad) {
        // Set random loading messages
        setRandomLoadingMessages();
        ...
        setTimeout(() => {
            ...
            if (loadingOverlay) {
                ...
                // Mark that we've shown the overlay in this session
